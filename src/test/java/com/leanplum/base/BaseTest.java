...
public class BaseTest {
	...
	private List<TestDevice> connectedTestDevices;
	private static TestConfig testConfig;
	private SoftAssert softAssertion;
	private String testIdentifier;
	private Map<TestDevice, Integer> testDevicePortMap = new HashMap<>();
	...
	public void setupAppiumService() {
		...
		for (TestDevice testDevice : connectedTestDevices) {
			System.out.println("ID: "+testDevice.getId());
			testDevicePortMap.put(testDevice, Integer.valueOf(findFreePort()));
			System.out.println("PORT: " + testDevicePortMap.get(testDevice));
			services.add(AppiumServiceUtils.setupAppiumService(platform, testDevicePortMap.get(testDevice)));
		}

//		for (int i = 0; i < connectedTestDevices.size(); i++) {
//			currentPorts.add(i, findFreePort());
//			System.out.println("PORT: " + currentPorts.get(i));
//			this.service = AppiumServiceUtils.setupAppiumService(platform, currentPorts.get(i));
//		}
	...
	public void setupDriver() {
		...
		// TODO device name!!!!!
		for (TestDevice testDevice : connectedTestDevices) {
			System.out.println("Platform is: " + testDevice.getPlatform().getPlatformName().toLowerCase());

			this.driver = df.createDriver(
					testDevice, DevicePropertiesUtils
							.getDeviceProperties(testDevice.getPlatform().getPlatformName().toLowerCase(), "phone"),
					testDevicePortMap.get(testDevice));
		}
	}
